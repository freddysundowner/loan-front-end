...
export class AddLoanApplicationComponent implements OnInit, OnDestroy {
  ...
  interestType = "";
  interestTypeId = "";
  processingFeeTypeId: any;
  processingFeeValue: any;
  ...
  public filteredServerSideMembers: ReplaySubject<any> = new ReplaySubject<any>(
    1
  );
  ...
  ngOnInit() {
      ...
      .pipe(
        ...
        map((search) => {
          ...
          return this.members.filter((member) => {
            return (
              member.first_name.toLowerCase().indexOf(search) > -1 ||
              ...
              member.account?.account_number.toLowerCase().indexOf(search) >
                -1 ||
    ...
    this.firstFormGroup = this.fb.group({
      ...
      repayment_period: [{ value: "", disabled: false }],
    ...
    this.paymentMethodService.list(["name", "display_name"]).subscribe(
      (res) => (this.paymentMethods = res),
      () => (this.paymentMethods = [])
    );

    this.witnessTypeService.list(["name", "display_name"]).subscribe(
      (res) => (this.witnessTypes = res),
      () => (this.witnessTypes = [])
    );
  }
   ...
   */
  onItemChange(value) {
    this.nationalID = this.members.find(
      (item: any) => item.id === value
    ).id_number;
    this.accountNumber = this.members.find(
      (item: any) => item.id === value
    ).account.account_number;
    this.memberAccounts = this.accounts.filter(
      (item: any) => item.member_id === value
    );
    this.clientPhone = this.members.find(
      (item: any) => item.id === value
    ).phone;
    // const mpesaNumber = this.clientPhone.slice(3);
    const mpesaNumber = this.clientPhone;
  ...
  onLoanTypeItemChange(value) {
    this.interestRate = this.loanTypes.find(
      (item: any) => item.id === value
    ).interest_rate;
    this.interestType = this.loanTypes.find(
      (item: any) => item.id === value
    ).interest_type.display_name;
    this.interestTypeId = this.loanTypes.find(
      (item: any) => item.id === value
    ).interest_type.id;

    this.penaltyTypeId = this.loanTypes.find(
      (item: any) => item.id === value
    ).penalty_type_id;
    this.penaltyValue = this.loanTypes.find(
      (item: any) => item.id === value
    ).penalty_value;
    this.penaltyFrequencyId = this.loanTypes.find(
      (item: any) => item.id === value
    ).penalty_frequency_id;

    this.repaymentPeriod = this.loanTypes.find(
      (item: any) => item.id === value
    ).repayment_period;
    this.repaymentFrequency = this.loanTypes.find(
      (item: any) => item.id === value
    ).payment_frequency.display_name;
    this.repaymentFrequencyId = this.loanTypes.find(
      (item: any) => item.id === value
    ).payment_frequency.id;

    this.reducePrincipalEarly = this.loanTypes.find(
      (item: any) => item.id === value
  ...
  create() {
    ...
    body.processing_fee_type_id = this.processingFeeTypeId;
    body.processing_fee_value = this.processingFeeValue;

    body.mpesa_number = this.secondFormGroup.get(
      "mpesa_fields.mpesa_number"
    ).value;
    body.mpesa_first_name = this.secondFormGroup.get(
      "mpesa_fields.mpesa_first_name"
    ).value;
    body.mpesa_last_name = this.secondFormGroup.get(
      "mpesa_fields.mpesa_last_name"
    ).value;

    body.cheque_number = this.secondFormGroup.get(
      "bank_fields.cheque_number"
    ).value;
    body.cheque_date = this.secondFormGroup.get(
      "bank_fields.cheque_date"
    ).value;
    body.bank_name = this.secondFormGroup.get("bank_fields.bank_name").value;
    body.bank_branch = this.secondFormGroup.get(
      "bank_fields.bank_branch"
    ).value;
    ...
    this.loader = true;
    this.loanApplicationService.create(formData).subscribe(
      (res) => {
        this.onSaveComplete();
        this.notification.showNotification(
          "success",
          "Success !! New Loan Application."
        );
      },
      (error) => {
        this.loader = false;
        ...
        if (this.formErrors) {
          ...
          for (const prop in this.formErrors) {
            ...
            if (this.secondFormGroup.controls[prop]) {
              ...
              this.secondFormGroup.controls[prop].setErrors({
                incorrect: true,
              });
            }
