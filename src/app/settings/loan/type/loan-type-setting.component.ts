...
export class LoanTypeSettingComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.service.dependencies(['name', 'display_name'])
            .subscribe((res) =>  {
                    this.interestTypes = res.interest_types;
                    this.paymentFrequencies = res.payment_frequencies;
                    this.penaltyTypes = res.penalty_types;
                    this.penaltyFrequencies = res.penalty_frequencies;
                    this.loanCharges = res.extra_charges;
